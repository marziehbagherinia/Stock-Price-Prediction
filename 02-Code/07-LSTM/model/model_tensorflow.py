...
class Model:
    ...
    def placeholders(self):
        self.X = tf.compat.v1.placeholder(tf.float32, [None,self.config.time_step,self.config.input_size])
        self.Y = tf.compat.v1.placeholder(tf.float32, [None,self.config.time_step,self.config.output_size])

    def net(self):
        def dropout_cell():
            basicLstm = tf.compat.v1.nn.rnn_cell.LSTMCell(self.config.hidden_size)
            dropoutLstm = tf.compat.v1.nn.rnn_cell.DropoutWrapper(basicLstm, output_keep_prob=1-self.config.dropout_rate)
            return dropoutLstm

        cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([dropout_cell() for _ in range(self.config.lstm_layers)])
        output_rnn, _ = tf.compat.v1.nn.dynamic_rnn(cell=cell, inputs=self.X, dtype=tf.float32)

        # shape of output_rnn is: [batch_size, time_step, hidden_size]
        self.pred = tf.compat.v1.layers.dense(inputs=output_rnn, units=self.config.output_size)

    def operate(self):
        self.loss = tf.reduce_mean(tf.square(tf.reshape(self.pred, [-1]) - tf.reshape(self.Y, [-1])))
        self.optim = tf.compat.v1.train.AdamOptimizer(self.config.learning_rate).minimize(self.loss)
        self.saver = tf.compat.v1.train.Saver(tf.compat.v1.global_variables())

def train(config, train_and_valid_data):
    tf.compat.v1.disable_eager_execution()
    with tf.compat.v1.variable_scope("stock_predict"):
        model = Model(config)
...
def predict(config, test_X):
    config.dropout_rate = 0     # Forecast mode should be adjusted to 1

    tf.compat.v1.reset_default_graph()    
    with tf.compat.v1.variable_scope("stock_predict", reuse = tf.compat.v1.AUTO_REUSE):
        model = Model(config)
